...
- Download recovery retains only complete confirmed packets. After the packet transport's three attempts fail, the app performs a short three-attempt `IDENTIFY` check: no valid identity immediately selects the disconnected view, while a valid identity triggers one automatic retry from the retained byte offset before the file is reported as needing operator retry.
- Download recovery retains only complete confirmed packets. After the packet transport's three attempts fail, the app performs a short three-attempt `IDENTIFY` check: no valid identity immediately selects the disconnected view, while a valid identity triggers one automatic retry from the retained byte offset before the file is reported as needing operator retry.
