...
- Initial serial setup is now a flat, wordmark-only screen containing just the port selector and Continue action. The default output folder has moved to App Settings. When no gauge responds, the normal shell collapses to a minimal `Disconnected` state with a subtle sequential pulse around the official Northstar mark and a small settings menu for recovery.
